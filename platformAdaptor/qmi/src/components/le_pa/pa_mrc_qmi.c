...
#if defined(NAS_PCI_SCAN_LIST_MAX_V01)
            return NAS_SCAN_TYPE_PCI_V01;
#else
            break;
#endif
    }
...
//--------------------------------------------------------------------------------------------------
/**
 * Initialize a pa_mrc_PlmnInformation_t
 *
 */
//--------------------------------------------------------------------------------------------------
static void InitializePlmnInformation
(
    pa_mrc_PlmnInformation_t* plmnInformationPtr    ///< [IN] the Plmn information to initialize.
)
{
    memset(plmnInformationPtr,0,sizeof(*plmnInformationPtr));
    plmnInformationPtr->link = LE_DLS_LINK_INIT;
}

//--------------------------------------------------------------------------------------------------
/**
 * Initialize a pa_mrc_PciScanInformation_t
 *
 */
//--------------------------------------------------------------------------------------------------
static void InitializePciScanInformation
(
    pa_mrc_PciScanInformation_t* scanInformationPtr    ///< [IN] the Scan information to initialize.
)
{
    memset(scanInformationPtr,0,sizeof(*scanInformationPtr));
    scanInformationPtr->link = LE_DLS_LINK_INIT;
...
)
{
    ...
    PlmnInformationPool=le_mem_CreatePool("PlmnInformationPool", sizeof(pa_mrc_PlmnInformation_t));
...
(
    le_dls_List_t *scanInformationListPtr ///< [IN] list of pa_mrc_PciScanInformation_t
)
{
...
)
{
    ...
    if (performNetworkScanResp.scan_result_valid)
    {
        ...
        {
            LE_ERROR("Network scan didn't complete due to a radio link failure recovery");
            return LE_COMM_ERROR;

        } else if (performNetworkScanResp.scan_result==NAS_SCAN_AS_ABORT_V01)
        {
            LE_ERROR("Network scan aborted");
            return LE_FAULT;
        }
    }
    ...
    if (performNetworkScanResp.pci_plmn_info_valid)
    {
        pa_mrc_PciScanInformation_t* newScanInformation = NULL;
        int i, j;
        nas_pci_scan_cell_info_type_v01* ptr = performNetworkScanResp.pci_plmn_info.pci_cell_info;

        for (i=0; i<performNetworkScanResp.pci_plmn_info.pci_cell_info_len; i++)
        {
            newScanInformation = le_mem_ForceAlloc(PciScanInformationPool);
            if(NULL == newScanInformation)
            {
                return LE_FAULT;
            }
            InitializePciScanInformation(newScanInformation);
            le_dls_Queue(scanInformationListPtr,&(newScanInformation->link));
            LE_DEBUG("The current cell id is = %"PRIu16"", ptr[i].cell_id);
            newScanInformation->cellId = ptr[i].cell_id;

            LE_DEBUG("Create PlmnInformationList !! %i",i);
            newScanInformation->plmnList = LE_DLS_LIST_INIT;
            for (j=0; j<performNetworkScanResp.pci_plmn_info.pci_cell_info[i].plmn_len; j++)
            {
                LE_DEBUG("Allocate memory for PlmnInformation %i",j);
                pa_mrc_PlmnInformation_t* newPlmnInformation = NULL;
                newPlmnInformation = le_mem_ForceAlloc (PlmnInformationPool);
                if(NULL == newPlmnInformation)
                {
                    return LE_FAULT;
                }
                InitializePlmnInformation(newPlmnInformation);
                le_dls_Queue(&newScanInformation->plmnList,&(newPlmnInformation->link));
                LE_DEBUG("PlmnInformation added in the list of plmninformation");
                ConvertMccMncIntoStringFormat(
                                                  ptr[i].plmn[j].mcc,
                                                  ptr[i].plmn[j].mnc,
                                                  ptr[i].plmn[j].mnc_includes_pcs_digit,
                                                  mccStr,
                                                  mncStr);
                LE_DEBUG("pci.plmn.mcc = %i", ptr[i].plmn[j].mcc);
                LE_DEBUG("pci.plmn.mnc = %i", ptr[i].plmn[j].mnc);
                memcpy(newPlmnInformation->mobileCode.mcc,mccStr,LE_MRC_MCC_BYTES);
                memcpy(newPlmnInformation->mobileCode.mnc,mncStr,LE_MRC_MNC_BYTES);
            }
        }
    }
#endif
