 ...
 */
//--------------------------------------------------------------------------------------------------
static void GetApnConfig
(
    swi_legato_avms_get_setting_resp_msg_v01* respPtr
)
{
    int32_t index;
    le_result_t result;
    ...
    if (!respPtr)
    {
        LE_WARN("null response pointer");
        return;
    }

    memset(&respPtr->apn_config, 0, sizeof(apn_config_type_v01));

    index = le_data_GetCellularProfileIndex();
    pRef = le_mdc_GetProfile((uint32_t) index);
    if (!pRef)
    {
        LE_WARN("failed to get profile reference");
        return;
    }

    result = le_mdc_GetAPN(pRef, respPtr->apn_config.apn, APN_MAX_LEN_V01);
    if (LE_OK != result)
    {
        LE_WARN("failed to get apn: %s", LE_RESULT_TXT(result));
        return;
    }
    respPtr->apn_config.apn_len = strlen(respPtr->apn_config.apn);

    result = le_mdc_GetAuthentication(pRef, &auth, respPtr->apn_config.uname, UNAME_MAX_LEN_V01,
                                      respPtr->apn_config.pwd, PWD_MAX_LEN_V01);
    if (LE_OK != result)
    {
        LE_WARN("failed to get authentication: %s", LE_RESULT_TXT(result));
        return;
    }
    respPtr->apn_config.uname_len = strlen(respPtr->apn_config.uname);
    respPtr->apn_config.pwd_len = strlen(respPtr->apn_config.pwd);

    respPtr->apn_config_valid = 1;
...
)
{
    ...
    if (LE_OK == (result = le_avc_GetUserAgreement(LE_AVC_USER_AGREEMENT_CONNECTION, &bPromptUser)))
    {
    ...
    if (LE_OK == (result = le_avc_GetUserAgreement(LE_AVC_USER_AGREEMENT_DOWNLOAD, &bPromptUser)))
    {
    ...
    if (LE_OK == (result = le_avc_GetUserAgreement(LE_AVC_USER_AGREEMENT_INSTALL, &bPromptUser)))
    {
    ...
    if (LE_OK == (result = le_avc_GetUserAgreement(LE_AVC_USER_AGREEMENT_REBOOT, &bPromptUser)))
    {
        resp.prompt_reboot_valid = 1;
        resp.prompt_reboot = bPromptUser;
    }
    else
    {
        LE_WARN("le_avc_GetUserAgreement LE_AVC_USER_AGREEMENT_REBOOT failed %d", result);
        // reboot prompt TLV is optional, no need to flag failure on response
    }
 ...
 */
//--------------------------------------------------------------------------------------------------
static void SetApnConfig
(
    apn_config_type_v01* cfgPtr
)
{
    int32_t index;
    le_result_t result;
    ...
    if (!cfgPtr)
    {
        LE_WARN("null APN config pointer");
        return;
    }
    ...
    result = le_mdc_SetAPN(pRef, cfgPtr->apn);
    if (LE_OK != result)
    {
        LE_WARN("failed to set apn: %s", LE_RESULT_TXT(result));
        return;
    }

    // FIXME setting username & password requires authentication type which is not
    // available in the QMI spec FH #41111626
    LE_INFO("only APN is set, username & password are currently ignored");
...
)
{
    ...
    if (settingPara.prompt_reboot_valid)
    {
        LE_INFO("setting prompt reboot");
        if ((result = le_avc_SetUserAgreement( LE_AVC_USER_AGREEMENT_REBOOT,
                        settingPara.prompt_reboot)))
        {
            LE_ERROR("le_avc_SetUserAgreement LE_AVC_USER_AGREEMENT_REBOOT failed %d", result);
            failed = true;
        }
    }

    if (settingPara.apn_config_valid)
    {
        SetApnConfig(&settingPara.apn_config);
    }
